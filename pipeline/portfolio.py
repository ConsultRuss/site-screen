...
def allocate(
    ...
    for pid, e, v in elig:
        ...
        row = {
            ...
            # option_cost = the econ record's control_cost, renamed for portfolio-layer semantics
            "option_cost": e["control_cost"],
            "diligence_total": e["diligence_total"],
...
def build_portfolio(
    ...
    """One allocation per configured budget + an echoed assumptions block for the UI."""
    pcfg = cfg["deal_economics"]["portfolio"]
    return {
        "_note": "Generated by pipeline/portfolio.py — illustrative option-budget allocation "
                 "over the pursue/pursue-if shortlist (passes excluded). Option capital deployed "
                 "now; diligence staged on the top speed-to-power winners as power validates. "
                 "Authored rationale in deal-notes.json. Synthetic + illustrative.",
        "assumptions": {
            "budgets_usd": pcfg["budgets_usd"], "stage_top_n": pcfg["stage_top_n"],
            "priority": pcfg["priority"], "exclude_verdicts": pcfg["exclude_verdicts"],
            "disclosure": pcfg["disclosure"],
        },
        "allocations": [
            allocate(economics_parcels, verdicts_by_id, cfg, b) for b in pcfg["budgets_usd"]
        ],
    }


def write_portfolio(cfg: dict[str, Any], economics_path, verdicts_path, out_path) -> int:
    """Read economics.json + verdicts.json, compute, write portfolio.json. Returns budget count."""
    import json
    from pathlib import Path

    econ = json.loads(Path(economics_path).read_text(encoding="utf-8"))["parcels"]
    vlist = json.loads(Path(verdicts_path).read_text(encoding="utf-8"))["verdicts"]
    verdicts_by_id = {v["parcel_id"]: v["verdict"] for v in vlist}
    data = build_portfolio(econ, verdicts_by_id, cfg)
    Path(out_path).write_text(json.dumps(data, ensure_ascii=False, indent=0), encoding="utf-8")
    return len(data["allocations"])
